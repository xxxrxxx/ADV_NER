...
def compute_f1(results):
    output_file= 'E:/xr/俺的毕设/AT4ChineseNER-master/AT4ChineseNER-master/data/output_sighan.txt'
...
def main(_):
    ...
    setting = GRUsighan_model.Setting()
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            with tf.variable_scope('ner_model'):
                m = GRUsighan_model.TransferModel(setting, tf.cast(embedding, tf.float32), adv=True, is_train=False)
                m.multi_task()
            saver=tf.train.Saver()
            for k in range(3500,12500,500):
