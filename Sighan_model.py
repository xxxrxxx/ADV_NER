...
class TransferModel(object):
    ...
    def multi_task(self):
        ...
        with tf.variable_scope('common_bilstm', reuse=tf.AUTO_REUSE):
            shared_cell_fw1 = tf.contrib.rnn.LSTMCell(self.lstm_dim)
            shared_cell_bw1 = tf.contrib.rnn.LSTMCell(self.lstm_dim)
            if self.is_train:
                shared_cell_fw1 = tf.nn.rnn_cell.DropoutWrapper(shared_cell_fw1, input_keep_prob=self.in_keep_prob,
                                                               output_keep_prob=self.out_keep_prob)
                shared_cell_bw1 = tf.nn.rnn_cell.DropoutWrapper(shared_cell_bw1, input_keep_prob=self.in_keep_prob,
                                                               output_keep_prob=self.out_keep_prob)
            (output_fw1, output_bw1), _ = tf.nn.bidirectional_dynamic_rnn(
                shared_cell_fw1, shared_cell_bw1, input, sequence_length=self.sent_len, dtype=tf.float32)
            shared_output1 = tf.concat([output_fw1, output_bw1], axis=-1)
            #第二层
        #with tf.variable_scope('common_bilstm2'):
            shared_cell_fw2 = tf.contrib.rnn.LSTMCell(self.lstm_dim)
            shared_cell_bw2 = tf.contrib.rnn.LSTMCell(self.lstm_dim)
            if self.is_train:
                shared_cell_fw2 = tf.nn.rnn_cell.DropoutWrapper(shared_cell_fw2, input_keep_prob=self.in_keep_prob,
                                                                output_keep_prob=self.out_keep_prob)
                shared_cell_bw2 = tf.nn.rnn_cell.DropoutWrapper(shared_cell_bw2, input_keep_prob=self.in_keep_prob,
                                                                output_keep_prob=self.out_keep_prob)
            (output_fw2, output_bw2), _ = tf.nn.bidirectional_dynamic_rnn(
                shared_cell_fw2, shared_cell_bw2, shared_output1, sequence_length=self.sent_len, dtype=tf.float32)
            shared_output2 = tf.concat([output_fw2, output_bw2], axis=-1)
            shared_output=self.self_attention(shared_output2)
            shared_output_2 = tf.expand_dims(shared_output,axis=-1)
            # shared_output = tf.reshape(shared_output,[-1,80,280,1])
            max_pool_output = tf.nn.max_pool(shared_output_2, ksize=[1, self.num_steps, 1, 1],
                                           strides=[1, self.num_steps,1, 1], padding='SAME')
            max_pool_output = tf.reshape(max_pool_output, [-1, 2 * self.lstm_dim])

        with tf.variable_scope('cws_private_bilstm', reuse=tf.AUTO_REUSE):
            cws_private_cell_fw1 = tf.contrib.rnn.LSTMCell(self.lstm_dim)
            cws_private_cell_bw1 = tf.contrib.rnn.LSTMCell(self.lstm_dim)
            if self.is_train:
                cws_private_cell_fw1 = tf.nn.rnn_cell.DropoutWrapper(cws_private_cell_fw1, input_keep_prob=self.in_keep_prob,
                                                               output_keep_prob=self.out_keep_prob)
                cws_private_cell_bw1 = tf.nn.rnn_cell.DropoutWrapper(cws_private_cell_bw1, input_keep_prob=self.in_keep_prob,
                                                               output_keep_prob=self.out_keep_prob)

            (output_fw1, output_bw1), _ = tf.nn.bidirectional_dynamic_rnn(
                cws_private_cell_fw1, cws_private_cell_bw1, input, sequence_length=self.sent_len, dtype=tf.float32)
            cws_private_output1 = tf.concat([output_fw1, output_bw1], axis=-1)
        #with tf.variable_scope('cws_private_bilstm2'):
            #第二层lstm
            cws_private_cell_fw2 = tf.contrib.rnn.LSTMCell(self.lstm_dim)
            cws_private_cell_bw2 = tf.contrib.rnn.LSTMCell(self.lstm_dim)
            if self.is_train:
                cws_private_cell_fw2 = tf.nn.rnn_cell.DropoutWrapper(cws_private_cell_fw2,
                                                                     input_keep_prob=self.in_keep_prob,
                                                                     output_keep_prob=self.out_keep_prob)
                cws_private_cell_bw2 = tf.nn.rnn_cell.DropoutWrapper(cws_private_cell_bw2,
                                                                     input_keep_prob=self.in_keep_prob,
                                                                     output_keep_prob=self.out_keep_prob)

            (output_fw2, output_bw2), _ = tf.nn.bidirectional_dynamic_rnn(
                cws_private_cell_fw2, cws_private_cell_bw2, cws_private_output1, sequence_length=self.sent_len, dtype=tf.float32)
            cws_private_output2 = tf.concat([output_fw2, output_bw2], axis=-1)
            cws_private_output=self.self_attention(cws_private_output2)

        with tf.variable_scope('ner_private_bilstm', reuse=tf.AUTO_REUSE):
            ...
            (output_fw1, output_bw1), _ = tf.nn.bidirectional_dynamic_rnn(
                ner_private_cell_fw1, ner_private_cell_bw1, input, sequence_length=self.sent_len, dtype=tf.float32)
            ner_private_output1 = tf.concat([output_fw1, output_bw1], axis=-1)

            #第二层lstm
        #with tf.variable_scope('ner_private_bilstm'):
            ner_private_cell_fw2 = tf.contrib.rnn.LSTMCell(self.lstm_dim)
            ner_private_cell_bw2 = tf.contrib.rnn.LSTMCell(self.lstm_dim)
            if self.is_train:
                ner_private_cell_fw2 = tf.nn.rnn_cell.DropoutWrapper(ner_private_cell_fw2,
                                                                     input_keep_prob=self.in_keep_prob,
                                                                     output_keep_prob=self.out_keep_prob)
                ner_private_cell_bw2 = tf.nn.rnn_cell.DropoutWrapper(ner_private_cell_bw2,
                                                                     input_keep_prob=self.in_keep_prob,
                                                                     output_keep_prob=self.out_keep_prob)

            (output_fw2, output_bw2), _ = tf.nn.bidirectional_dynamic_rnn(
                ner_private_cell_fw2, ner_private_cell_bw2, ner_private_output1, sequence_length=self.sent_len, dtype=tf.float32)
            ner_private_output2 = tf.concat([output_fw2, output_bw2], axis=-1)
            ner_private_output=self.self_attention(ner_private_output2)
